...
for page in range(parsing):
    ...
    for city, link, salary, company in zip(list_city, list_link, list_salary,
        ...
        link_response = requests.get(link, headers=headers_gen.generate())
        html_item = link_response.text
        hh_main_link = BeautifulSoup(html_item, 'lxml')
        key_skills = hh_main_link.find_all('div', class_='bloko-tag-list')
        for data in key_skills:
            print(data.text)
            if "Django" in data.text and "Flask" in data.text:
                dict_job = {
    ...
    with open('File information.json', 'a', encoding='utf-8',
              newline="") as file:
